...
- [USER-DECISION→DS-1 **已定案 2026-07-18**] Windows 支持范围与时间点(传输 named pipe vs loopback + signal 路径语义 §13.1 + peer identity + 支持阶段;architecture §18-Q3;§15 登记);**裁决=正式采默认安全侧:Windows 全不在支持矩阵,恒 [UNSUPPORTED](需求出现再 reopen 本条,§15 裁决记录)**。
...
- **cutover fence 接线(B1,armed 时)**:新注册读取 cutover fence revision 并调用 `coverageVerified`(architecture §16.1(三)(四);§17.4);无 coverage 的新注册停在 ATTACHED。[B1-Q3][D-2]
...
- **cutover fence 接线(B1,armed 时)**:resume/clear/compact 滚代读取同一 cutover fence revision 并调用同一 `coverageVerified`;既有 session 滚代后无 coverage 置正交 `cutoverEligibility=blocked_drain_required`(architecture §16.1(四);§17.4)。[B1-Q3][D-2]
...
| T2 | submitted | eligibility 评估(accept 同事务或恢复补跑) | !cancelRequested(置位则径 T14) | queued | queueReason ∈ {ready, target_busy, awaiting_wake, budget_wait(+nextEligibleAt), **provider_wait(Chunk 4 增补 v0.10.6:circuit 专属,T4d)**, **cutover_blocked(B1 §17 armed 期 cutover fence guard;additive 增补)**} |
| T3 | queued | `claim` CAS(pull) | `queueReason=ready 且 now ≥ max(nextEligibleAt, retryNotBeforeAt)(NULL=不限)`;offer 有效且 `offer.offeredStateRevision == task.stateRevision == request.expectedStateRevision`;**offer.candidateOwner == 调用会话(四层+generation)且 Task resolved target == 该会话**(route ACL,Chunk 3 增补);**rs execution slot guard(轮 24 同步)**:当前 {rs_, generation} 无 held slot **且同 rs_ 任意 generation 无 poisoned_unresolved slot**(session 级隔离,§6.1;冲突→4013);**数据治理 fence(Chunk 6 v0.12.5-v0.12.8,§5.1 fence③(a)+④):effectiveDataGovernance.governancePolicyEpoch==目标 room 当前 ∧ **重解析成功且 governanceResolveDigest==tasks.governanceResolveDigest**(轮 59-60 P1-F/P1-6:含 endpoint.regionRevision/grant.version;**重解析失败(grant 过期/撤销/缺失/region mismatch)或 digest 不等 → 不 dispatch,走 T21 superseded(resolve_invalid)**)**;!cancelRequested;**书挡② reserve 成功**(attempt 消耗/concurrency/context/cost,与 T4 共用同一 dispatch-bookend reducer,architecture §6.6);**B1 cutover fence guard(armed 时)**:目标 session 满足 `coverageVerified(session, epoch, fenceRevision)`(architecture §16.1(三))且无 `cutoverEligibility=blocked_drain_required`;不满足 → 不 dispatch,任务保持 queued 并置 `queueReason=cutover_blocked`+`nextEligibleAt` 重排(coverage/drain 达成后由 janitor 重评) | leased | 首次 dispatch 时 CAS TaskRouteOwnership;offer 消费+sibling revoke+DispatchAttempt+ExecutionLease(含 dispatchId)+completionTokenHash+**claim_deliveries 行(claimRequestId 幂等,§5.3)**;payload 随同步响应返回(非 outbox) |
| T4 | queued | `reserve_push_dispatch`(push) | **逐字枚举(不继承 T3 的 offer/调用会话条件——push 无 ClaimOffer)**:`queueReason=ready 且 now ≥ max(nextEligibleAt, retryNotBeforeAt)`;Task resolved target == 该 worker rs_;**worker rs execution slot guard 同 T3(当前 gen 无 held 且任意 gen 无 poisoned,§6.1)**;**数据治理 fence 同 T3(§5.1 fence③(a)+④:room epoch ∧ governanceResolveDigest 重验;push 到 codex_worker 是跨厂商主路)**;route CAS;书挡② reserve 成功(同 reducer);!cancelRequested。**T4 所建 lease 初始 status=active**(push 无 pending_ack 阶段);**B1 cutover fence guard(armed 时)**:目标 session 满足 `coverageVerified(session, epoch, fenceRevision)`(architecture §16.1(三))且无 `cutoverEligibility=blocked_drain_required`;不满足 → 不 dispatch,任务保持 queued 并置 `queueReason=cutover_blocked`+`nextEligibleAt` 重排(coverage/drain 达成后由 janitor 重评) | dispatching | DispatchAttempt+ExecutionLease+completionTokenHash+holder tuple **持久化**+`dispatch_intent` outbox(intent_committed;`dispatchAckDeadlineAt` 写入);**PushCompletionHandle 为 commit 后进程内构造物,非事务产物** |
...
| cutover closure/snapshot invariant breach 补扫 | armed fence 存在 | `cutover_closure_breach_repair`(§17.3;只修 breach 不推进 phase)[E-1] |
| cutover_blocked 重评 | 目标 session coverage/drain 状态变化(coverageVerified 变真或 fence superseded) | 调用 §17.3 cutoverEligibility 重算 reducer(两支分立:仅 coverage 成立支回填 evidence,superseded 支只清旗——窄审 r4 修复③)——清旗+queueReason→ready 同事务;stale fence 不清新旗(窄审 r1 修复 D2-6) |
| abort attempt 恢复 | `cutover_abort_attempts.state=pending_reconciliation` | **补发前按 durable cause 三查(r10-b1 修②+r11-b1 修③+r12-b1 修③):(i) 同 attempt registry winner→引导 §17.3 段② superseded 收口支(不补发 stop-outbox);(ii) obligation `resolvedByRef`=`catr_`→幂等 rollback 支收口;(iii) `cerr_` 行被 supersede 的 exact pair(`cutoverEpoch`+`supersededFenceRevision`)与 obligation 所属 `(cutoverEpoch,fenceRevision)` 逐字段全等→幂等 emergency 支收口(epoch-only presence 不得收口——r13-b1/RECOVERY);generic「epoch superseded」无 cause ref 不得收口;(ii)(iii) 恒不向已 superseded epoch 补发;皆无→**补发未发 interrupt/drain outbox、等待停止 evidence;evidence 到达后引导 §17.3 abort 段② 终局事务,janitor 不自行终局(窄审 r2 修复②) |
...
- **B1 cutover 表族(`legacy_cutover_state` 等 21 表)见 §17.1,规范地位同本节**(表本体挂 §17.1,照 §10.13『§6.1 之外表清单』先例)。
...
| B1 cutover:arm | §17.3(B1 族通则:admin receipt 消费为各 writer 内联子步骤,不单列) |
| B1 cutover:armed 期增量(两分支) | §17.3 |
| B1 cutover:transition CAS | §17.3 |
| B1 cutover:emergency revoke | §17.3 |
| B1 cutover:rollback 段① | §17.3 |
| B1 cutover:rollback 段② | §17.3 |
| B1 cutover:reconcile_effect 事实 | §17.3 |
| B1 cutover:abort_effect 段① | §17.3 |
| B1 cutover:abort_effect 段② | §17.3 |
| B1 cutover:rollback_cutover | §17.3 |
| B1 cutover:cutoverEligibility 重算 | §17.3 |
| B1 cutover:legacy partition 冻结 | §17.3 |
| B1 cutover:resolution conflict audit-only writer | §17.3 |
...
task.submit {
  ...
  request: {                                  # caller 列族(§6.2),全部 caller-authored
    ...
    dataClassification: "unspecified"|"restricted",   # DS-2 已定案(2026-07-18):二值即正式词表(§15 裁决记录);restricted→I-4 保守侧
...
- **source-event tuple 不入本表(B1;§18-Q9)**:EffectContext 于 claim/dispatch 期建立、一 context 可挂多个以 toolUseId 区分的 intent,不是 per-source-event carrier,不承载认证 source-event 四元组 {authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel, sourceEventId};其 durable per-event carrier=effect_intents 四列(§12.3);EffectPermit/EffectAttempt 经 intentRef join,不重复落列。
...
effect_intents efi_<ulid> {
  ...
  authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel, sourceEventId,
                                       # B1 per-event source-event 四元组(broker-owned 持久列,
                                       #   required 非空;认证 pep/intent 创建事务自 authctx/认证
                                       #   通道 exact-copy;caller/payload 零写入权,wire body 出现
                                       #   同名字段 → forbidden_field(§12.2 通则);EffectPermit/
                                       #   EffectAttempt 经 intentRef join 不重复落列;cutover 枚举
...
#     不误 join)**, **authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel,
#     sourceEventId(B1 四元组入判同:同 context/attempt/toolUseId、同操作语义但异 source event
#     的重送不静默 replay 旧行——tuple 任一字段不同即异 digest,恒沿既有 4063
#     effect_intent_conflict+审计、原行不动,live 与终态 tombstone 两支同规则;live 部分唯一键
#     不扩,不允许同 toolUseId 双行)**,
#   (列持久;domain 入 §16.3)。
...
| `effect_intents` | effectIntentId(efi_) | operationHash 索引;**(effectContextId, taskId, dispatchId, attempt, toolUseId) 部分唯一(live,轮 42 B3+轮 44 context 入键)**;canonicalArgs/canonicalArgsDigest/**effectSemanticDigest/supersededByIntentId(轮 44)**;state 封闭 CHECK(**含 settled 终态**);**B1 四列 {authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel, sourceEventId}(broker-owned;入 effectSemanticDigest 判同,live 唯一键不扩;§17.1 obligations 枚举消费)** |
...
### 14.1 探测项登记表(对齐 architecture §15 Phase 0A 现有十二项——本表登记其中十一项:第 1-9 项 P0A-1..9 对齐其编号;第 10 项 P0A-CB 系本文起草、architecture v0.10/A-9 回填单列;第 12 项 P0A-2b 随 B1 冻结稿新增;第 11 项 P0A-1b 留 B2 冻结稿登记)
...
| **P0A-2b** | §15 Phase 0A 第 12 项(hook trust/loader 状态可取证性——保持纯平台探测:逐 host/profile/version 记录 exact trusted digest、per-session load boundary 是否可由宿主所有的证据证明、证据 producer/freshness/拒绝面;PASS/FAIL/UNMEASURABLE 只供 B1-Q1 选择预先封闭分支,不自动启用 cutover)[共识冻结=codex-r3:83] | §17.1 `hook_load_proof_profiles`、architecture §16.1(三) loadProof allowed set | HookLoadProofEvidence |
...
```text
HookLoadProofEvidence {                # P0A-2b
  probeContractDigest: "sha256:…",     # 探测契约冻结锚(执行前冻结;变化=新 profileVersion 重探)
  branches: [{kind: "direct"|"new_session_after_trust",
              branchStatus: "PASS"|"FAIL"|"UNMEASURABLE",
              unmeasurableReason: "no_evidence_surface"|"no_authenticated_producer"|"host_blocks_probe"|null,
                                     # 仅 UNMEASURABLE 非空(tagged CHECK);封闭 reason 集 [spec 期实现细节](窄审 r1 修复 D2-8)
              exactTrustedDigestObserved: bool,   # 逐 host/profile/version 的 exact trusted digest 可记录
              loadBoundaryProvable: bool,         # per-session load boundary 可由宿主所有的证据证明
              evidenceProducer: string, freshnessBasis: string, rejectionSurface: string
             }](恰 2,kind UNIQUE、按字典序)
}
```
...
- **branch 三值判定(窄审 r1 修复 D2-8:proof 可用性与测量运行完整性分离)**:branch PASS ⇔ `exactTrustedDigestObserved ∧ loadBoundaryProvable ∧ evidenceProducer/freshnessBasis/rejectionSurface 非空且可复现`;branch FAIL ⇔ 判据可评估且任一为假;branch UNMEASURABLE ⇔ 测量面本身不可行——**必须携封闭 `unmeasurableReason` 与完整 attempt/artifact/producer/freshness/rejection-surface 记录**。三值均要求测量尝试记录完整;任一支缺运行必需 evidence → 顶层不得 PASS(INCONCLUSIVE)。**[spec 期实现细节]**
- probe 顶层 status=PASS ⇔ `passPredicate=true` = 两支判定均完整可复核(含带 reason 的 UNMEASURABLE)——严格照 baseline `status=PASS ⇔ passPredicate=true`、exact-shape 八者与消费门/重抽验义务;branch 的 UNMEASURABLE **不是**顶层新增状态。**两支皆 FAIL/UNMEASURABLE(记录完整)仍可整体 PASS**——条件式参数语义:该 host 的 cutover 恒停在 legacy_note_only 之前,是合法参数结论而非设计失败(此语义保留已获窄审 r1 §2 确认)。
- 回填:`HookLoadProofProfileV1` 派生=`allowedLoadProofKinds := 按字典序 [branchStatus=PASS 的 kind]`,仅对应 branch PASS 才入集,FAIL/UNMEASURABLE 均不入集;空集=合法(coverageVerified 恒 false、相关 cutover 边 fail-closed);host/profile/probe contract 变化只 append successor profile revision 并重探;**PASS 不自动 enable cutover**(§14.3 回填协议「conjunct-eligible 非 auto-enable」通则逐字适用)。[共识冻结=codex-r17:40]

---

## 15. 决策登记册(decision register;全文 USER-DECISION 汇总)

> 本章汇总全文 [USER-DECISION→DS-n] 条目,供用户逐项裁决;每条给**选项+推荐+默认安全侧**(定案前实现按默认安全侧或不实现,§1.2)。**本章结构化呈现选项并记录用户裁决;未定案项不替用户决策**。

| DS-n | 决策点 | 选项 | 默认安全侧(定案前) | 关联条 |
|---|---|---|---|---|
| **DS-1 ✅已定案(2026-07-18)** | Windows 支持范围与时间点(architecture §18-Q3 全决策,非仅 transport) | (a) 传输:named pipe / loopback TCP(带鉴权);**且**须一并定:signal-file 路径语义(§13.1 STATE_DIR/路径分隔)、peercred 等价的 peer identity 机制、支持阶段 | Windows 全不在支持矩阵(§2.1;恒 UNSUPPORTED)——**裁决=正式采此侧** | §2.1/§13.1、architecture §18-Q3 |
| **DS-2 ✅已定案(2026-07-18)** | 数据治理词表+carrier 粒度+room 治理默认(architecture §10.2:task/room 携 dataClassification+accountClass+provider/region allowlist) | dataClassification 二值/细分;room accountClassPolicy(allow_unknown/require_known)+providerRegionAllowlist 默认充填 | dataClassification 二值(restricted→I-4 全覆盖);accountClass=unknown 不处理公司数据;allowlist 缺省=空(跨厂商派发 fail-closed)——carrier 已由 effectiveDataGovernance(§6.1)承载;**裁决=正式采此三元组** | §7.2/§6.1、architecture §10.2 |
| **DS-3(本文新增;2026-07-18 用户暂缓)** | context-basis 开放策略(仅 P0A-CB PASS ∧ architecture §9.1 修订 ∧ feature context_current_v1 三前置齐后) | 开放 generation_only / 开放 digest_bound / 保持 fresh-only | 保持 current v1 [UNSUPPORTED](§9.1)——**暂缓:维持 PENDING 与此默认侧,后续另行裁决** | §9.2、§9.3、§14.1 P0A-CB |

- **裁决记录(2026-07-18 会话 12;用户原文「DS-1 采默认定案,DS-2 采默认定案,DS-3 暂缓」)**:DS-1=正式采默认(Windows out-of-scope 恒 [UNSUPPORTED];未来需求出现再 reopen 本条,reopen 须一并解决选项列全部连带项);DS-2=正式采默认三元组(dataClassification 二值/accountClass=unknown 不处理公司数据/providerRegionAllowlist 缺省空 fail-closed);DS-3=暂缓(维持 PENDING,定案前行为继续按默认安全侧 fresh-only;后续另行裁决)。**DS-1/DS-2 裁决值与定案前默认安全侧完全一致;DS-3 未定,定案前行为继续按默认安全侧——零 wire 语义变化**;效力变化=DS-1/DS-2 从「定案前按默认安全侧」转「已裁决正式值」(实现依据从临时转正式),DS-3 维持 PENDING。
...
| 4065-4070 | (reserved) | **B4 arbitration dispute 族预留(共识已冻结:4065 dispute_not_found…4070 unsupported_question_schema,均 retryable=false;随 B4 冻结稿落表)**——B1 自 4071 起分配 | false |
| 4071 | `cutover_coverage_required` | §17/architecture §16.1(四)(armed 期新注册 READY 阻挡/dispatch guard 失败;+remediation=取得 coverage 或 drain) | false |
| 4072 | `cutover_transition_conflict` | §17.2(expectedTransitionRevision 陈旧,first-winner 已 commit) | false |
| 4073 | `cutover_gate_receipt_invalid` | §17.2 C1(F-1 member receipt 缺失/五元组绑定不符/trust revision 与 architecture §16.1(二)(三)不同) | false |
| 4074 | `cutover_policy_snapshot_mismatch` | §17.3(G-3 invariant breach:state/receipt/policy row 三方不等或悬空 ref;hold+fail-closed+审计) | false |
| 4075 | `admin_action_receipt_invalid` | §17.3(AdminOperatorActionReceipt 过期/撤销/scope 或依赖 digest 漂移/重复消费) | false |
| 4076 | `cutover_rollback_verification_failed` | §17.2 C4/C5(legacy 完整性验证任一失败;保持原 phase+emergencyHold,deny-only) | false |
| 4077 | `cutover_edge_forbidden` | §17.2(封闭边集外的 (from,to),含一切 stood_down 回边) | false |
| 4078 | `cutover_resolution_conflict` | §17.1 registry 行/§17.3(different-result resolution 提交:败者整事务回滚+audit-only writer 提交 `crca_` 行后才返回;响应携 auditRef/winnerResolutionRef)(r10-b1 修③) | false |
...
| `AgentBridge/EffectSemantic/v1` | effectSemanticDigest(intent 重放判同)(B1:hash-input 增 per-event 四元组维=11 字段;GV=附录 GV-8) | §12.3 |
...
| `AgentBridge/RequiredAdapterGateSet/v1` | `requiredAdapterGateSetDigest`=AB-CANON-1(本 domain,`{members:["claude_clean_profile_wake_gate_v1","claude_pep_deny_path_gate_v1"]}`)(architecture §16.1(九)公式逐字)(地位:**[共识冻结=F-1;B1-Q7 验收包登记义务]**) | §17.1 |
| `AgentBridge/HookCoverageBinding/v1` | `coverageEvidenceDigest`=AB-CANON-1(本 domain, `{installationId, cutoverEpoch, runtimeSessionRecordId, runtimeSessionGeneration, endpointId, endpointGeneration, sessionIdentityEvidenceRef, requiredHookSetDigest, observedHookSetDigest, hookConfigDigest, hostTrustRevision, loadProof, challengeNonce, receivedAtBroker, validatedAtBroker, freshUntilBroker, observedAtHost}`)——D-1 修正后 17 字段封闭表,optional/可空一律显式 null(窄审 r1 修复 D2-10)(地位:**[spec 期实现细节:字段序窄审确认;命名已获窄审 r1 §2 agree 保留]**) | §17.1 |
| `AgentBridge/CutoverTransitionReceipt/v1` | `transitionReceiptDigest`=AB-CANON-1(本 domain, 基础 14 字段+`rollbackReason`(前进边显式 null)+`epochPolicyRef`+`evaluatedPolicySnapshotDigest`——17 字段封闭表,optional 显式 null)(SEC-7 ②审计链可重放锚;保留裁定=窄审 r1 §2 agree,「若删」条件句废除)(地位:**[spec 期实现细节:字段序窄审确认]**) | §17.1 |
| `AgentBridge/CutoverEpochPolicy/v1` | `evaluatedPolicySnapshotDigest`=AB-CANON-1(本 domain, `{cutoverEpoch, acceptedHookSetDigest, targetHookSetDigest, requiredHookSetDigest, requiredAdapterGateSetDigest, hookConfigDigest, fenceRevision, policyRevision}`——8 字段封闭表)(G-3 强制存在的 digest 的已登记域;不用未注册裸 hash,窄审 r1 修复 D2-10;8 字段语义集已获窄审 r2 agree)(地位:**[spec 期实现细节:窄审确认]**) | §17.1 |
| `AgentBridge/LegacyEffectIntentPartition/v1` | `frozenMemberSetDigest`=AB-CANON-1(本 domain, `{members:[effectIntentId…]}`——成员=冻结时点四列全 NULL 行全体、按字典序)(H-6 ③ boundary/成员集审计复核锚)(地位:**[spec 期实现细节:窄审 r7 修复②]**) | §17.1+§17.5 H-6 |

- **嵌套形状封闭(窄审 r2 修复④,两实现无猜测复现)**:①`loadProof` 入 hash/列形=tagged `{kind:"direct", hostLoaderEvidenceRef, loadBoundaryId}` | `{kind:"new_session_after_trust", sessionStartEvidenceRef, trustedRevisionRef}`——discriminant=`kind`,每支字段集封闭、无缺省/可选字段;②`epochPolicyRef` 入 hash/wire 形=object `{installationId, cutoverEpoch, policyRevision}`(JCS 三字段);③`adapterGateReceiptRefs[]` 数组序=按 member 字典序(与 F-1 members 数组同序,恰两项);④GV 与逐字段 mutation fixtures 同步覆盖 loadProof 两支、epochPolicyRef object 与数组序。**[spec 期实现细节]**

---

## 17. 旧链退场 cutover(§18-Q9/B1;architecture §16.1 的协议细化) [NORMATIVE]

> 语义 SSOT=architecture §16.1;本章只补齐存储表族、封闭转移边、writer 事务、控制面与 migration,不重定义语义。SEC-7 gate 纪律见 architecture §16.1(九)。

### 17.1 表族(PK/唯一约束/关键列;retention class 逐表标注)

| 表 | 主键 | 唯一/CHECK/关键列 | retention |
|---|---|---|---|
| `legacy_cutover_state` | installationId(每安装单行) | 列=architecture §16.1(一)`LegacyCutoverStateV1` 全 11 字段 **[共识冻结]** + `epochPolicyRef`(→cutover_epoch_policies;G-3);`phase` CHECK ∈ {dual_gate, v3_verified, legacy_note_only, stood_down};`emergencyHold` tagged CHECK=G-2 逐字(normal_rollback ⇒ revokeTrustRevisionRef 恒 NULL;emergency_revocation ⇒ NOT NULL 且指向 exact `hook_trust_revisions` 行);四 digest 列仅判定时点只读 snapshot/cache、禁独立 setter(G-3) | authority 面恒 durable |
| `cutover_epoch_policies` | (installationId, cutoverEpoch, policyRevision) | append-only(演进只 append successor revision,G-3);列=`{cutoverEpoch, acceptedHookSetDigest, targetHookSetDigest, requiredHookSetDigest, requiredAdapterGateSetDigest, hookConfigDigest, fenceRevision, policyRevision}`(B1-Q4a 五字段与 G-3 SSOT 四 digest 之并集,见 architecture §16.1(一))+ createdAt/adminReceiptRef **[spec 期实现细节:并集列序]** | authority 面 |
| `hook_trust_revisions` | (installationId, trustRevision) | append-only;`status` CHECK ∈ {staged, active, revoked};列 ⊇ `{trustRevision, digest, status, activatedAt?, revokedAt?, reason, adminReceiptRef}` **[共识冻结=B1-Q4a]**;staged 永不满足 coverage;revoked 经 join 立即 stale | authority 面 |
| `hook_coverage_evidence` | evidenceId(`hcov_`) | 列=architecture §16.1(三)字段族 **[共识冻结+D-1 取代形;窄审 r1 修复 D2-1]**:freshness 判定字段=`receivedAtBroker/validatedAtBroker/freshUntilBroker`,旧 `observedAt/freshUntil` **不落列**,`observedAtHost` 仅审计;`coverageEvidenceDigest` 列(域=§16.3 `AgentBridge/HookCoverageBinding/v1`);`loadProof` tagged CHECK 封闭两支 exact 形=`{kind:"direct", hostLoaderEvidenceRef, loadBoundaryId}` \| `{kind:"new_session_after_trust", sessionStartEvidenceRef, trustedRevisionRef}`(与 §16.3 嵌套形状封闭一致——窄审 r3 修复④,消除 branch-label 简记与 tagged object 并存);**append-only、无 `current` 列**——currentness=coverageVerified join 判定(全部 exact 条件 ∧ freshUntilBroker 未过期 ∧ validatedAtBroker 最大,并列以 evidenceId 定序;删派生真值位,窄审 r1 修复 D2-11);索引 `(runtimeSessionRecordId, runtimeSessionGeneration, cutoverEpoch, validatedAtBroker)` | 被 snapshot member `coverageEvidenceRef`/transition 审计链 live 引用行=authority safe-delete 豁免;未引用行按 evidence 面(窄审 r1 §3 position 采纳;ref carrier 已落) |
| `cutover_fences` | (installationId, cutoverEpoch, fenceRevision) | B1-Q3 四元组 `cutoverFence{epoch,revision,requiredHookSetDigest,mode=armed}` **[共识冻结]** 的列级落地(列名 cutoverEpoch/fenceRevision 及 mode 抽列=**[spec 期实现细节]**);`mode` CHECK ∈ {armed, superseded}(superseded=rollback/新 epoch 收口 **[spec 期实现细节]**);`legacyResolutionRefs[]`(r7-b1 修①;r8-b1 修③:引用对象=`effect_attempt_resolutions` winner 行,按 resolutionId 字典序冻结、无=空数组;纳入 C2 `drainClosureDigest` 输入)**[spec 期实现细节]**;arm 事务先持久本行再枚举 snapshot(§17.3) | authority 面 |
| `cutover_coverage_snapshots` | (installationId, cutoverEpoch, inventoryRevision) | 列=architecture §16.1(四)`CutoverCoverageSnapshotV1` 全 7 字段 **[共识冻结]** | authority 面 |
| `cutover_snapshot_members` | (installationId, cutoverEpoch, inventoryRevision, runtimeSessionRecordId, runtimeSessionGeneration) | 成员集明细(lifecycle≠OFFLINE 当前代+overlap-risk 旧代);`drainEvidenceRefs`(→closure/receipt 行;drainComplete 恒为 architecture §16.1(四)四合取谓词的判定结果,不设独立可写 bool 真值列)+ `coverageEvidenceRef?`(→hook_coverage_evidence;coverage evidence 的 live-reference 边=retention split 载体,窄审 r1 修复 D2-7)——**adoption/write owner(窄审 r2 修复③,r3 收窄 scope)**:①cutoverEligibility 重算 reducer 清旗事务 ②**仅 C2 transition CAS**(唯一实际消费 per-member coverageVerified 的边;C1/C3/C4/C5 的 guard 不消费 per-member coverage,**不回填**)——二者在 durable 判定实际消费 coverageVerified 时,把本次判定所选 evidence 行 **NULL→ref write-once CAS** 回填(same-ref replay=no-op;更换采纳须新 inventoryRevision 语境,不改写既有 ref);**active snapshot 唯一选择规则(窄审 r3 修复③)**:每 `(installationId, cutoverEpoch, fenceRevision)` 的 active `inventoryRevision`=最大值(append-only,arm/重建只 append 新 revision);reducer/C2 的写 target=该 active revision 的 member 行,CAS binding 含 `{cutoverEpoch, fenceRevision, inventoryRevision}` 三元;GC 的「active snapshot」按同一规则唯一选择;审计链冻结时点=transition receipt commit;**存在 ref 为 NULL 的 active snapshot member 期间,未引用 coverage evidence 的 GC 不启用(activation fence,fail-safe 方向=不删)** **[spec 期实现细节]** | authority 面 |
| `cutover_abort_attempts` | attemptId(`caat_`) | abort_effect 段① durable 载体(窄审 r2 修复②+r3 补 fence):`{obligationRef, adminActionReceiptRef(已消费 exact receipt), **stopTarget**(exact stop fence:`{taskId, dispatchId, attempt, leaseId, leaseEpoch, effectAttemptRef?, permitRef?, targetRuntimeSessionRecordId, targetGeneration}`——恢复补发恒指同一 execution,窄审 r3 修复②), **outboxOperationKey/outboxRef**(interrupt/drain outbox 幂等绑定), state ∈ {pending_reconciliation, closed}, startedAtBroker, closedAtBroker?, completionKind?, winnerResolutionRef?, cutoverAttemptReceiptRef?, emergencyRevokeRef?}`;部分唯一 `(obligationRef) WHERE state='pending_reconciliation'`(每 obligation 至多一 live abort attempt);**终局 encoding(r10-b1 修②+r11-b1 修①+r12-b1 修①:四 durable 终局)**:`completionKind` CHECK ∈ {aborted, superseded_by_winner, superseded_by_cutover_rollback, superseded_by_emergency_revocation}(state=closed ⇔ NOT NULL,pending 恒 NULL);**成对 CHECK(kind⇔cause ref 一一强制)**:`winnerResolutionRef`(→`effect_attempt_resolutions` 同 installation 同 attempt winner 行)仅 superseded_by_winner 非空;`cutoverAttemptReceiptRef`(→`cutover_attempt_receipts` 同 installation `catr_` 行,且=关联旧 obligation `resolvedByRef` 所指同一行)仅 superseded_by_cutover_rollback 非空;`emergencyRevokeRef`(→`emergency_rollback_receipts` 同 installation `cerr_` 事实行,且=supersede 关联 obligation 所属 `(cutoverEpoch,fenceRevision)` 的 exact revoke 事实行——机械判定=exact pair join:`cerr_.cutoverEpoch`(pre-increment 旧值)=obligation 所属 cutoverEpoch ∧ `cerr_.supersededFenceRevision`=obligation 所属 fenceRevision ∧ 同 installation(r13-b1/TERMINAL:epoch-only presence 不充当 cause;UNIQUE(installationId, cutoverEpoch) 只保证每 epoch 至多一行,不证明 fence 对应))仅 superseded_by_emergency_revocation 非空;aborted 三 ref 恒 NULL、pending 四列恒 NULL;**generic「epoch 已 superseded」无 cause ref 不得收口(r12-b1 修③)**;恢复 owner=启动恢复扫描/janitor 按 pending 行续跑——**每次补发前按 durable cause 三查(r11-b1 修③+r12-b1 修③):(i) 同 attempt registry winner 在场→引导 §17.3 段② superseded 收口支(不补发 stop-outbox);(ii) 关联 obligation `resolvedByRef`=`catr_` 行→幂等 CAS closed(superseded_by_cutover_rollback+该 exact `catr_`);(iii) 关联 obligation 所属 `(cutoverEpoch,fenceRevision)` 已被 emergency revocation supersede(判定=同 installation `cerr_` 行 exact pair 逐字段全等:`cutoverEpoch`+`supersededFenceRevision` 与 obligation 二元完全相等;epoch-only presence 不得收口——r13-b1/RECOVERY)→幂等 CAS closed(superseded_by_emergency_revocation+该 exact `cerr_`);(ii)(iii) 恒不向已 superseded epoch 补发;三者皆无→按 outboxOperationKey 幂等补发、等待停止 evidence,不自行终局**;**pending 终止源=判据式封闭(r12-b1 修⑤)**:winner(reconcile)∪ 停止 evidence(abort-win)∪ **epoch/fence-superseding writer 封闭集**(=§17.2/§17.3 一切递增 epoch/fence 的 writer,G-3 合成句同一封闭集,现枚举恰二:`rollback_cutover`、emergency revoke)逐一对应 cause 分支;未来新增此类 writer 须同批增 completionKind cause 支——closed⇔cause ref 成对 CHECK 强制,无 cause 者不可收口,fail-safe 方向=滞留可检、非误收口**[spec 期实现细节]** | authority 面 |
| `cutover_source_event_obligations` | obligationId(`cobl_`) | **UNIQUE(installationId, cutoverEpoch, fenceRevision, authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel, sourceEventId)**——pre-fence/in-flight source-event obligation 逐条明细(architecture §16.1(四);窄审 r1 修复 D2-3);**枚举输入封闭映射(窄审 r2 修复①,r3 修正 carrier 基数)**:(a) `approval_records`(pep 通道)行——四元组逐列同名直取(§11.3 UNIQUE 同构;命中与未命中均有行,故 pep 通道事件全可枚举),取「尚无对应 closure/终局」者;(b) live EffectIntent/EffectPermit/EffectAttempt——**per-event carrier=`effect_intents` 增持久四元组列 `{authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel, sourceEventId}`**(认证 pep/intent 创建事务 exact-copy;per-event——EffectContext 在 claim/dispatch 期建立、一 context 可挂多个以 toolUseId 区分的 intent,非合法 per-source-event carrier,不承载;Permit/Attempt 经 intentRef join)——**§12.3 `effect_intents` additive 增列+replay 判同+§12.10 表清单与 §12.2 接线=实际 exact delta 见差分 H(窄审 r4 修复①②;本表行仅为索引)**;`resolvedByRef?` **NULL→ref 单向 CAS(write-once;same-ref replay=no-op;different-ref=invariant conflict+审计;窄审 r2 SPEC-DETAIL/RESOLVED-BY-REF)**,允许 target kind 封闭={`ccls_`,`crcr_`,`cacr_`,`catr_`};`catr_` 支可被多条旧 epoch obligation 引用(**不设全局 UNIQUE**),反向绑定=一对一支 receipt 行携 `obligationRef`、`catr_` 支携 `unresolvedObligationRefs[]`;obligation 全量终局入 `drainClosureDigest` 与 C2 guard | authority 面 |
| `cutover_effect_closures` | closureId | **UNIQUE(cutoverEpoch,fenceRevision,runtimeSessionRecordId,runtimeSessionGeneration,sourceChannel,sourceEventId)**(E-1 六列逐字 **[共识冻结]**,两路写入幂等锚);列=architecture §16.1(五)`CutoverEffectClosureV1` 全 12 字段 **[共识冻结]**;`decision` CHECK='closed_no_effect';`approvalRecordRef`→approval_records(§11.3,四元组 correlation 精确相等) | authority 面 |
| `admin_operator_action_receipts` | receiptId | `kind` CHECK ∈ {emergency_rollback, reconcile_effect, abort_effect, rollback_cutover}(共识枚举=`EmergencyRollback\|reconcile_effect\|abort_effect\|rollback_cutover` **[共识冻结=清单 r9:23/D-3]**,首项 wire 形取 snake_case——**命名归一注记,窄审确认**);AdminReceiptDisciplineV1 六条的列级落地:installationId/scopeDigest/依赖 digest+revision 列族/reason/issuedAtBroker/expiresAtBroker(有限)/revocation state+ref/nonce/revision/consumedAt?+consumedByRef?(一次性消费);本表 append-only+`receiptDigest` 列(条④ append-only 审计并产 durable receipt digest 的落地);消费前依赖漂移即失效(§17.3;条⑥经 4075 fail-closed 落地) | authority 面 |
| `emergency_rollback_receipts` | receiptId | 事实 receipt(E-2 双层之事实层);列=`{adminPrincipal, cutoverEpoch, revokedDigest, expectedLegacyConfigDigest, reason, issuedAt}`(B1-Q4b 六字段 **[共识冻结]**;**`cutoverEpoch` 语义锁=被本次 revoke supersede 的 pre-increment 旧 epoch**——冻结字段名保留、以语义注记消歧,writer 事务断言写入值=事务起点现行 epoch、post-increment 值恒拒——r13-b1/TERMINAL)+ **`installationId`(明列——r13-b1/TERMINAL)**+ **`supersededFenceRevision`(被 supersede 的 pre-increment 旧 fence;与 `cutoverEpoch` 合成被 supersede 的 exact pair carrier;FK/CHECK 绑定同 installation exact `cutover_fences` 行 `(installationId, cutoverEpoch, supersededFenceRevision)`——r13-b1/TERMINAL)**+ `adminActionReceiptRef`(E-2)+ 验证 evidence refs+**UNIQUE(installationId, cutoverEpoch)**(epoch 单调递增、每 epoch 至多一次 revoke 事实——约束保持、不因 pair 列可组合三列 UNIQUE 而放松(冗余不附加);exact cause 判定不依赖本 UNIQUE、依赖 pair 列逐字段全等——r12-b1 修①③+r13-b1/TERMINAL)**[spec 期实现细节]**;不因授权 receipt 事后 expiry/revoke 失效 | authority 面 |
| `reconciliation_closure_receipts` | receiptId | 列=`{obligationRef, effectDisposition, evidenceRefs[], closedAtBroker}`(B1-Q2b **[共识冻结]**)+ `adminActionReceiptRef`(E-2)+**resolution anchor(r7-b1 修①;r8-b1 修②④:fact 行恒 immutable、winner 经 registry、schema 封闭无隐式第三形)**:`effectAttemptRef` **NOT NULL**(pep 通道事件不产生 `crcr_`——无 attempt 行即无 unknown,其终局=closure 行,**显式排除于本 schema**)+`resolutionAnchor` tagged CHECK 封闭两支=`{kind:"source_event", exactSourceTuple:{authenticatedPrincipalId, originRuntimeSessionRecordId, sourceChannel, sourceEventId}}`(v11 行;写入事务自 attempt 经 intentRef join 的 `effect_intents` 四列 exact-copy,tuple 任一分量≠intent carrier 四列=写入拒;同 installation CHECK)\|`{kind:"legacy_partition", legacyFreezeReceiptRef}`(legacy-v7 行;→`effect_intent_legacy_freeze_receipts` 同 installation;attempt 经 intentRef join 的 intent 须∈该 receipt 冻结的全 NULL 分区成员集);`obligationRef` **创建时定值、本表恒不 UPDATE**(armed 语境=必填直绑;pre-arm 语境=NULL 恒 NULL——adoption 不回写本表,write-once 回填面=`effect_attempt_resolutions` registry 行);`effectDisposition` CHECK ∈ {applied_settled, not_applied}(源句『只有这两值满足转移屏障』的字面指代=effect fact 值 {reconciled_settled, closed_no_effect}(architecture §16.1(六)逐字保留);本行为其 disposition 列级投影 **[spec 期实现细节,窄审确认]**) | authority 面 |
| `abort_closure_receipts` | receiptId | 终局 `aborted_no_effect` 才写行(append-only resolution fact 层,不改写 raw attempt 记录);`abort_pending_reconciliation` 为动作返回态,**不落终局 receipt——pending 由 `cutover_abort_attempts`+outbox+audit 承载**(窄审 r3 措辞修正)**[spec 期实现细节]**;+ `adminActionReceiptRef`;**armed-only(r8-b1 修①:pre-arm conclusive set 收窄为 reconcile-only)**:`abort_effect` 前置=cutover obligation 在场(armed 语境),本表 `obligationRef` 恒必填、`caat_` pending 载体恒可用;pre-arm 历史 unknown 的唯一 operator 处置路径=`reconcile_effect`(`crcr_`),**不存在 pre-arm `cacr_`**;`effectAttemptRef` NOT NULL+同事务 registry insert(winner 纪律同 reconciliation 行) | authority 面 |
| `cutover_attempt_receipts` | receiptId | 列=`{outcome, cutoverEpoch, unresolvedObligationRefs[], rollbackPolicyRef, closedAtBroker}`(B1-Q2b `CutoverAttemptReceiptV1` **[共识冻结]**)+ `adminActionReceiptRef`;`outcome` CHECK='rolled_back_unresolved_effect'(cutover-attempt outcome,非 Task outcome——不扩 §6.1 architecture 封闭枚举) | authority 面 |
| `required_adapter_gate_receipts` | receiptId | `member` CHECK ∈ {claude_clean_profile_wake_gate_v1, claude_pep_deny_path_gate_v1}(F-1 字典序封闭集 **[共识冻结]**);绑定五元组列=`{cutoverEpoch,hookTrustRevision,gateProfileId,conformanceSuiteDigest,canaryCohortDigest}` **[共识冻结]**(呈现统一取 codex-r9:34 无空格形,与 architecture 稿(九)一致;清单 r9:27 为带空格同一元组——源排版分歧注记);**UNIQUE(installationId, cutoverEpoch, member)**(每 member 恰一 receipt ref) | authority 面 |
| `cutover_transition_receipts` | receiptId(`ctrx_`) | 列=architecture §16.1(七)`CutoverTransitionReceiptV1` 基础 14 字段 **[共识冻结]** + `rollbackReason?`(G-2 载体;tagged CHECK:C4/C5 非空且=emergencyHold.reason=所消费 admin receipt reason,C1-C3 恒 NULL——窄审 r1 修复 D2-2)+ `epochPolicyRef`+`evaluatedPolicySnapshotDigest`(G-3)+ `transitionReceiptDigest`(域=§16.3);完整物理 schema=14+3+digest 列;**UNIQUE(installationId, cutoverEpoch, expectedTransitionRevision)**(CAS first-winner 锚);(fromPhase,toPhase) CHECK ∈ §17.2 封闭边集 | authority 面 |
| `hook_load_proof_profiles` | (hostMatrixRef, profileVersion) | 列=`HookLoadProofProfileV1={hostMatrixRef, profileVersion, probeContractDigest, allowedLoadProofKinds[], probeEvidenceDigest}` **[共识冻结=codex-r17:40]**;`allowedLoadProofKinds` CHECK ⊆ {direct, new_session_after_trust} 按字典序、仅对应 branch PASS 入集;append-only(host/profile/contract 变化只 append successor revision 并重探) | authority 面 |
| `effect_intent_legacy_freeze_receipts` | receiptId(`clfr_`) | H-6 legacy 分区冻结 receipt(r7-b1 修②):列=`{installationId, frozenAtBroker, frozenRowCount, frozenMemberSetDigest, minWriterFloor}`(**五列,installationId 明列——r8-b1 修⑤**;`frozenMemberSetDigest` 域=§16.3 `AgentBridge/LegacyEffectIntentPartition/v1`);**UNIQUE(installationId)**(每安装恰一行)+append-only;**linearization point=冻结事务 DB commit(receipt 行与 enforcement 同事务原子可见;`frozenAtBroker` 仅同事务审计时钟、非序锚——r8-b1 修⑥)**;boundary exact 定义载体(§17.5 H-6 ③);resolution anchor legacy 支引用 target;冻结事务与 DB enforcement 同事务(§17.3 增行)**[spec 期实现细节]** | authority 面 |
| `effect_attempt_resolutions` | resolutionId(`cear_`) | resolution winner registry(r8-b1 修②③;r9-b1 修①②:outcome contract+tagged cross-row binding 封闭):列=`{installationId, effectAttemptRef, factRef(→crcr_/cacr_), effectDisposition, resolutionAnchor(两支同 reconciliation 行), obligationRef?}`(PK+6 列);**UNIQUE(effectAttemptRef)**;由写 `crcr_`(pre-arm/armed)或 armed `cacr_` 的同一 writer 事务 insert;**tagged cross-row binding(fact/registry 写入及 NULL→ref adoption 时强制;存储完整性异常→consumer fail-closed)**:`factRef` 按 prefix 指向 exact `crcr_`/`cacr_` 行,`installationId`/`effectAttemptRef` 与 fact 行等值;`crcr_` 支 `effectDisposition` CHECK ∈ {applied_settled, not_applied} 且 `resolutionAnchor` canonical bytes=fact 行同名值;`cacr_` 支恒 `{effectDisposition=aborted_no_effect, resolutionAnchor.kind=source_event(四元组由 exact obligation/attempt join 派生并在 writer 事务断言), obligationRef 非空且=fact 行同名值}`(armed-only);**registry 创建时 `obligationRef`=fact 行同名值**(armed=同值非空;pre-arm `crcr_`=同为 NULL);此后 **NULL→ref 仅允许 pre-arm source_event 支的 arm adoption**(目标 obligation 同 installation/epoch/fence 且四元组=anchor;fact 行保持 NULL);legacy_partition 支 registry/fact `obligationRef` 恒 NULL;**replay outcome contract(r9-b1 修①:冻结 outcome、不预选实现——preflight 或竞争败者整事务回滚后重读 winner 均可)**:same-result 等价关系=exact `{effectAttemptRef, factRef 目标类(crcr_\|cacr_), effectDisposition, resolutionAnchor canonical bytes}` **四元全等**;same→返回原 winner 的稳定 fact/result,**零第二 durable fact**;different→竞争败者**整事务回滚**(零 fact、receipt 保持未消费)后由独立 audit-only writer 事务提交 `crca_` audit 行(§17.1/§17.3——r10-b1 修③:durable audit 的可执行 owner 与 commit 顺序),**audit commit 后才返回 typed `cutover_resolution_conflict`(4078,响应携 auditRef/winnerResolutionRef)**,原 winner 不动,同样零第二 fact;first-winner 竞争败者不得留下第二 fact 或任何 winner-side effect;guard (d)/arm adoption/legacy fence 只消费本表 winner;`obligationRef` NULL→ref=write-once CAS(state/mapping 表=合法回填面,fact 行恒不动)**[spec 期实现细节]** | authority 面 |
| `effect_resolution_conflict_audits` | auditId(`crca_`) | different-result resolution conflict 的 durable audit 载体(r10-b1 修③):列=`{installationId, effectAttemptRef, winnerResolutionRef(→effect_attempt_resolutions 同 installation 行), attemptedFactKind, attemptedEffectDisposition, attemptedResolutionAnchor(canonical bytes), operatorKind, adminActionReceiptRef(仅关联——该 receipt 随败者事务回滚保持未消费), observedAtBroker}`;`attemptedFactKind` CHECK ∈ {crcr_, cacr_};`operatorKind` CHECK ∈ {reconcile_effect, abort_effect}(现行封闭 writer 集内 conflict 支仅 reconcile_effect 可达——abort 段② 恒走 superseded/same 支(anchor=join 派生确定性),列仍落供审计完备);append-only、无 UNIQUE 合并(每次 conflict 返回各产一行=逐次审计语义);仅由 §17.3 resolution conflict audit-only writer 独立事务写入;typed 4078 响应携 `auditRef` **[spec 期实现细节]** | audit 面(append-only durable;非判定载体,无 live-ref 豁免需求) |

### 17.2 cutover phase 封闭转移表

| # | from | to | guard(全部同事务评估;G-3 policy snapshot 重算恒含) | 同事务副作用 |
|---|---|---|---|---|
| C1 | dual_gate | v3_verified | 两 F-1 member receipts exact(绑定五元组逐字段相等)+ deny-path/canary 合格;emergencyHold=NULL | transition receipt+transitionRevision++ |
| C2 | v3_verified | legacy_note_only | 全 inventory `observedHookSetDigest ∈ target` coverage + effect/drain closure 全过(drainComplete 四合取逐 member;closure 屏障成员全满足);emergencyHold=NULL | 同上 |
| C3 | legacy_note_only | stood_down | 观察窗/recorded-note 义务完成;emergencyHold=NULL。**point of no return:此后无任何回 legacy 边** | 同上;G-5 的外部 enable 前置自此满足(gate 本身另行单开关,architecture §15 G-5) |
| C4 | v3_verified | dual_gate | rollback:emergencyHold 已持久(reason ∈ G-2 两值)∧ exact legacy config/issuer 完整性验证成功(事实 receipt 在案) | transition receipt(adminActionReceiptRef 必填)+重开 admission |
| C5 | legacy_note_only | dual_gate | 同 C4(emergency 支另要求 `EmergencyRollbackReceiptV1` 事实链:恢复旧链→宿主/TCB evidence 验 exact legacy config、issuer/key、hook 加载与 protected inventory 全部成立) | 同 C4 |

- **封闭性**:以上为全部合法边;`stood_down` 出度=0。不在表内的 (from,to) 请求 → typed `cutover_edge_forbidden`(§16.2 增补)。[B1-Q5]
- **phase-preserving mutation**:emergencyHold 置位/解除、epochPolicyRef 随 successor revision 更新、fence/inventory revision 推进——均推进 `transitionRevision`,不改 phase;rollback 失败恒保持原 phase+hold(绝不谎标 dual_gate)。[B1-Q4b][G-2]
- **emergency revoke 原子事务(非 phase 边)**:`{digest→revoked, trustRevision++, cutoverEpoch/fenceRevision++, emergencyHold=armed}`(B1-Q4b 四元组 **[共识冻结]**);emergencyHold 的落列编码按 G-2 tagged 结构=`{reason:"emergency_revocation", adminActionReceiptRef, heldAtBroker, revokeTrustRevisionRef=本事务 revoke 行}`(G-2 为 B1-Q4b「armed」标量的后续收敛细化,两 qid 并标,非冲突)。[B1-Q4b][G-2]**epoch-changing writer 的 G-3 合成(窄审 r1 修复 D2-5)**:本事务与 rollback_cutover 等一切递增 epoch/fence 的 writer 同规则——同一 writer 事务 append successor `CutoverEpochPolicyV1` 行+更新 `epochPolicyRef`/四 digest snapshot+新 fence 行+obligation carry-forward;不得留 state 指向旧 policy 行的中间态(G-3 漂移=breach 非合法 hold)。[G-3]
- **CAS 纪律**:每边带 expectedTransitionRevision(共识 receipt 字段)[B1-Q5];first-winner/陈旧赢家 typed conflict(4072)为 CAS+expected revision 的引申语义 **[spec 期实现细节,窄审确认]**。

### 17.3 writer 事务与 janitor

| 写路径 | 事务定义 | qid |
|---|---|---|
| arm | 单 writer 事务:持久 `cutover_fences{…, mode=armed}` → 枚举 member set 写 snapshot → **枚举写入 `cutover_source_event_obligations` 逐条明细(封闭 INSERT…SELECT 输入;窄审 r1 修复 D2-3)** → 以 obligation 集 join 既存 `recorded_not_protected` 行 `INSERT … SELECT` 回填同 epoch closure(并写 `resolvedByRef`)→ **pre-arm resolution adoption(r7-b1 修①;r8-b1 修②:经 registry,fact 行不动)**:逐条新建 obligation 以四元组与 `effect_attempt_resolutions` winner 行(`resolutionAnchor.kind=source_event`)的 `exactSourceTuple` 精确匹配,命中即同事务写 registry 行 `obligationRef`(NULL→ref write-once CAS)+`obligation.resolvedByRef`=winner 的 `factRef`(NULL→ref CAS)——`crcr_` fact 行恒不 UPDATE → 枚举 legacy-v7 分区 unknown 的 registry winner 行按 resolutionId 字典序冻结入本 fence 行 `legacyResolutionRefs[]`(H-6 ④);commit 后 armed 才可见 | [B1-Q3][E-1][B1-Q2a] |
| armed 期增量(**两分支**;窄审 r3 修复①) | `pep.check` 写 approval row 的同一 writer 事务 piggyback——**not_protected 分支**:①obligation 行 insert(四元组新时;replay 幂等)②增量 closure ③同事务 `obligation.resolvedByRef=该 closure`(NULL→ref CAS);**deny/protected(或已有 live effect)分支**:仅 obligation insert,**保持 unresolved、不写 closure**(`cutover_effect_closures.decision` CHECK 仅允许 recorded_not_protected 的 closed_no_effect,不为 protected 伪造 closure;其终局经(六)三分支/effect settlement 的 resolution fact);六列 UNIQUE 幂等 | [E-1][B1-Q2a] |
| transition CAS | 同事务:expectedTransitionRevision CAS + G-3 policy snapshot 重算(state snapshot=receipt snapshot=exact referenced policy row,任一不等/悬空=invariant breach→hold+fail-closed,4074)+ 边 guard 全量谓词 + **C2 边逐 member `coverageEvidenceRef` 回填(NULL 时;仅 C2——其余边 guard 不消费 per-member coverage,不回填;窄审 r3 修复③)** + receipt 写入 | [B1-Q5][G-3] |
| emergency revoke | §17.2 原子四元组事务;**同事务内联 admin receipt 消费子步骤并冻结 `consumedByRef`=本次 revoke/hold(emergencyHold.adminActionReceiptRef 指向已消费 exact receipt;窄审 r3 修复②)**;同事务写 `cerr_` 事实行(E-2 事实层落点显式化,本事务即其唯一 writer——r12-b1 修①;**行内持久被 supersede 的 exact pair:`cutoverEpoch`=事务起点现行(pre-increment)epoch、`supersededFenceRevision`=同 fence、`installationId` 明列——写入断言 post-increment 值恒拒,r13-b1/TERMINAL**)+**按旧 `(cutoverEpoch,fenceRevision)` join 部分唯一索引 `(obligationRef) WHERE state='pending_reconciliation'` 枚举关联 pending `caat_`→逐行 CAS closed(`completionKind=superseded_by_emergency_revocation`, `emergencyRevokeRef`=本次 `cerr_` 行)+取消未发 stop-outbox(r12-b1 修②,同事务案——与 G-3 合成/obligation carry-forward 同 commit 原子收口零窗口;admin receipt 恒保持已消费→`caat_`,不退还不改绑;新 epoch 下如仍需 abort=新 receipt+新 `caat_`)** | [B1-Q4b][G-2][B1-Q2b] |
| rollback 两段 | 事务①:**内联 admin receipt 消费子步骤(一次性 CAS+全量重验)并冻结 `consumedByRef`=本次 hold(receipt↔hold 原子绑定;窄审 r3 修复②)**+持久 emergencyHold;段间执行恢复/验证(不改 phase);事务②验证全成立才 CAS 回 dual_gate;crash 于两段间→持久 hold 保持 admission closed | [B1-Q4b][D-3] |
| admin receipt 消费(**子步骤定义,恒被下列 operator writer 原子内联,非独立事务**——窄审 r2 修复②) | `admin_operator_action_receipts` 一次性消费 CAS(consumedAt write-once)+消费前重验 expiry/revocation/scope/依赖 digest 全部(漂移即失效,4075);绑定 exact receipt→本次动作(consumedByRef=动作产物行;**per-action 语境封闭(r10-b1 修①,按动作拆分)**:**reconcile_effect=本次 `crcr_` fact 行**(仅 winner 事务消费;registry replay/conflict 支整事务回滚⇒消费一并回滚、receipt 保持未消费——零悬空绑定,r9-b1 修①);**abort_effect=段① `caat_` 行**(段① 事务消费,绑定目标与 durable 载体同 commit 原子可见;段② 恒不再消费,其任何分支(abort-win/replay/superseded)均不回滚段①,receipt 恒保持已消费→`caat_`)) | [D-3][E-2] |
| reconcile_effect 事实事务 | **单一 writer 事务原子合成**:内联 admin receipt 消费子步骤+决定性 evidence 重验+**fact 分层(窄审 r3 修复②:baseline `effect_attempts` 无 expectedStateRevision 列、state 枚举={applied, not_applied, unknown} 且 exact-shape CHECK 冻结——operator 恒不改写 raw attempt 行/receipt)**:`crcr_`=**append-only resolution fact 层**,逐值冻结映射=raw applied(经决定性 evidence 确认)→`reconciled_settled` / raw not_applied→`closed_no_effect` / raw unknown→仅当决定性 postimage/provider receipt/mediator evidence 证明 applied|not_applied 时写对应 resolution fact,**原 raw receipt/unknown 记录保留不覆写**;pep 通道无 attempt 行=closure 行即 fact;**active(未 settle)attempt 需收口时只能调用 baseline settleEffectAttempt whole reducer(原子处理 Permit/Lease/Intent/Task),本事务不得旁路**+写 `ReconciliationClosureReceiptV1`(anchor 纪律=§17.1 行:armed 直绑 `obligationRef`;pre-arm 携 `resolutionAnchor`、`obligationRef` 恒 NULL——r8-b1 修②:fact 行恒不回填)+**同事务 insert `effect_attempt_resolutions` registry 行**(winner/replay outcome contract=§17.1 registry 行:same-result→整事务回滚零第二 fact、重读返原 winner 稳定 result,**admin receipt 消费随事务回滚=保持未消费**;different-result→整事务回滚零 fact、receipt 同保持未消费→独立 audit-only writer 事务提交 `crca_` 行、audit commit 后才返回 typed 4078(§17.3 行——r10-b1 修③);**仅 winner 事务消费 receipt,`consumedByRef`=本次 `crcr_` fact 行**——r9-b1 修①;**本事务不设 `caat_`-pending 互斥 guard(r10-b1 修②之②支):pending abort attempt 不阻挡 reconcile,先赢 winner 由 abort 段② superseded 支收口**)+(armed 语境)`obligation.resolvedByRef` NULL→ref CAS | [B1-Q2b][E-2] |
| abort_effect 段①(**armed-only,r8-b1 修①:前置=cutover obligation 在场;pre-arm unknown 唯一路径=reconcile_effect**) | **单一 writer 事务**:内联 admin receipt 消费子步骤(**consumedByRef=本 `caat_` 行,消费与 durable 载体同事务原子提交——r10-b1 修①**)+撤销未消费 authority(unused permit revoke 等)+**写 durable `cutover_abort_attempts` 行(`caat_`;state=pending_reconciliation)+interrupt/drain outbox(标准 durable 异步载体)**——crash 后由启动恢复扫描/janitor 按该行续跑(补发未发 outbox、等待停止 evidence);返回 `abort_pending_reconciliation`(窄审 r2 修复②:段①有持久载体与恢复 owner) | [B1-Q2b][E-2] |
| abort_effect 段②(终局;**段② 自身两支;supersession 终局(rollback/emergency)=各 epoch-writer 侧收口非本段分支——r10-b1 修①②+r11-b1 修①+r12-b1 修①**) | **单一 writer 事务,先查同 attempt registry winner**——**(a) abort-win 支(无 winner)**:exact 停止/not-applied evidence 重验+`cutover_abort_attempts` 行 CAS pending→closed(`completionKind=aborted`)+写 `AbortClosureReceiptV1`(armed-only 恒直绑 `obligationRef`——r8-b1 修①:无 pre-arm 支)+同事务 insert registry 行(first-winner;UNIQUE race loser 重读后按 (b) 支收口)+`obligation.resolvedByRef` CAS;`cacr_`=append-only resolution fact 层,映射=raw not_applied/已证未开始→`aborted_no_effect`,**原 raw 记录保留不覆写;active attempt 收口只经 baseline settleEffectAttempt whole reducer**——同 reconcile 行纪律,窄审 r3 修复②;**(b) superseded 支(winner 在场,same/different 皆然——r10-b1 修②)**:零 `cacr_`、零第二 durable fact、不返回 conflict(abort=处置意图非事实断言,superseded 非错误;不新增 effect 事实终局值、不扩 Task outcome——效果事实=winner 既有 fact):`caat_` CAS pending→closed(`completionKind=superseded_by_winner`, `winnerResolutionRef`=该 winner registry 行)+取消未发 stop-outbox 续跑+动作返回 superseded 终局(携 winnerResolutionRef/factRef)**[spec 期实现细节]**,obligation 终局由 winner fact 侧既有路径承载;**两支均不消费 receipt(段① 已消费→`caat_`,恒不回滚不退还——r10-b1 修①)**;closed 后重放=按 `completionKind` 幂等返原终局(aborted→原 `cacr_` winner;superseded_by_winner→原 winnerRef;superseded_by_cutover_rollback→原 `catr_` ref;superseded_by_emergency_revocation→原 `cerr_` ref——r11-b1 修①+r12-b1 修①);两支 CAS 失败=行已被并发收口(winner/rollback 侧)→重读 closed 行按同规则幂等返回——仅段②(a) 写事实行 | [B1-Q2b][E-2] |
| resolution conflict audit-only writer(r10-b1 修③;败者回滚后独立事务) | different-result 竞争败者整事务回滚(零 fact/registry/obligation 写入,receipt 消费一并回滚——reconcile 语境)后,同一 operator 调用栈以**独立 audit-only 事务**读已 commit winner 行(registry 行 immutable,audit 内容确定性)+写 `effect_resolution_conflict_audits` 行(§17.1),**audit commit 后才返回 typed `cutover_resolution_conflict`(4078,响应携 auditRef/winnerResolutionRef)**;本事务不消费 receipt、不写 fact/registry/obligation;crash 于 audit commit 前=动作未完成无响应,重试重走完整判定(再 conflict 再产行——append-only 逐次记录语义) | [B1-Q2b][E-1] |
| rollback_cutover 事务 | **单一 writer 事务原子合成**:内联 admin receipt 消费子步骤+写 `CutoverAttemptReceiptV1`(携 `unresolvedObligationRefs[]`)+递增 epoch/fence+**G-3 合成**(successor policy 行+state ref/snapshot)+unresolved obligation carry-forward(新 epoch/fence 下重建 obligation 行,原行 `resolvedByRef=catr_` 行;底层 effect fact 保持 reconciling)+**同事务枚举并收口关联 pending `caat_`(r11-b1 修②,取审方同事务案)**:按本次 `catr_` 行 `unresolvedObligationRefs[]` join 部分唯一索引 `(obligationRef) WHERE state='pending_reconciliation'` 枚举(每 obligation 至多一 live 行,枚举域确定)→逐行 CAS pending→closed(`completionKind=superseded_by_cutover_rollback`, `cutoverAttemptReceiptRef`=本次 `catr_` 行)+取消未发 interrupt/drain stop-outbox 续跑(按 outboxOperationKey)——原 admin receipt 恒保持已消费→`caat_`,不退还不改绑;新 epoch 下如仍需 abort=新 receipt+新 `caat_`(绑定 carry-forward 后的新 obligation)(窄审 r1 修复 D2-4/D2-5+r2 修复②) | [B1-Q2b][G-3] |
| cutoverEligibility 重算 reducer | 单 writer,CAS 比较现值 `{cutoverEpoch, fenceRevision}`(对 LegacyCutoverState/fence 现行值,不新增第二真值位):**两支分立(窄审 r4 修复③:superseded 支不 adoption)**——(a) coverage 成立支:目标 session 对 exact current fence 的 coverageVerified 变真 → **`runtime_sessions.cutoverEligibility` SET NULL(唯一编码;「或写封闭 eligible 值」备选删除——窄审 r2 agree-side 裁定)**+同事务把本次判定实际消费的所选 evidence 回填 **active inventoryRevision** 的 member 行 `coverageEvidenceRef`(NULL→ref CAS;CAS binding 含 {cutoverEpoch, fenceRevision, inventoryRevision} 三元——窄审 r3 修复③)+重评受影响 queued tasks(queueReason→ready);(b) fence superseded 支:exact current fence 已 superseded(rollback/新 epoch 收口)→ 仅 SET NULL 清旗+重评 queued tasks,**不写 `coverageEvidenceRef`**(本支不消费 coverageVerified、可无 selected evidence,不得向 write-once ref 回填);stale fence 事件不得清新 epoch 的旗(窄审 r1 修复 D2-6) | [B1-Q3][D-2] |
| legacy partition 冻结(一次性;§17.5 H-6 ③) | floor 生效后单 writer 事务:`count(*)` 四列全 NULL 行+`frozenMemberSetDigest` 实算+写 `effect_intent_legacy_freeze_receipts` 行(五列)+同事务安装 DB enforcement(全 NULL INSERT 恒拒/partial 恒拒/四列 UPDATE immutable);**linearization point=本事务 DB commit(`frozenAtBroker` 仅审计时钟——r8-b1 修⑥)**;先于首次 arm(非空 legacy 分区无 receipt 行→arm 恒拒,H-6 ④) | [B1-Q2a][窄审 r7 修复②] |
| janitor 增行(§5.6 表) | `cutover_closure_breach_repair`:扫描修复检测到的 closure/snapshot invariant breach;**janitor 只修 breach,不作为正常 owner,不推进 phase** | [E-1] |

### 17.4 控制面与投影(§7 接线)

- **零模型面 mutation**:cutover 的一切变更动作(arm/transition/rollback/emergency revoke/reconcile_effect/abort_effect/rollback_cutover)经 admin/control plane 发起,Task/model RPC 不得代交;§7.1 方法→authority 矩阵**不增行**,§7.9 投影矩阵**不投影任何 cutover 方法**(明确不投影规则——B1 不虚构模型面 mutation;cutover 是 admin/ops 面)。[D-3][codex-r17:48②][claude-r18:26]
- **读面**:phase/epoch/emergencyHold、accepted/target/observed coverage、两 member 状态、stale/uncovered generations、drain obligations、reconciling、G-5 hard-disable 与 remediation 经 doctor/console 呈现(architecture §16.1(九)③);doctor 不是 fence,不新增 §7 wire read 方法(将来若需,另行差分)。[B1-Q7③]
- **task 面接线**:见差分 B(§5.2 guard/§5.6 janitor);注册面(SessionStart/generation roll/rebind)按 architecture §16.1(四)接线点读取同一 fence revision 并调用同一 `coverageVerified`(§2.5/§4.4 各加一句短引用,不复述)。[B1-Q3]

### 17.5 migration(expand/contract 纪律=architecture §11)

- 全部新表 expand(无 contract);`runtime_sessions` 增 nullable 列 `cutoverEligibility`(NULL=非 armed 期默认合格;armed 期由 broker 置 `blocked_drain_required` 正交旗)**[spec 期实现细节]**;`tasks.queueReason` CHECK 扩展一值(见差分 B,additive)。
- `min_writer` floor:本批表/列生效后按既有惯例推进;旧 binary 拒启不重建空库(architecture §11)。
- backfill:无(新域;`legacy_cutover_state` 首行由首次 arm 前的 admin 初始化写入,初始 phase=dual_gate)。**[spec 期实现细节]**
- **broker ID 前缀注册(§1.3 增补;窄审 r1 修复 D2-11+r2 增一枚)**:`hcov_`(coverage evidence)/`cobl_`(source-event obligation)/`ccls_`(effect closure)/`ctrx_`(transition receipt)/`caar_`(admin operator action receipt)/`cerr_`(emergency rollback receipt)/`crcr_`(reconciliation closure)/`cacr_`(abort closure)/`catr_`(cutover attempt)/`caat_`(abort attempt,段① 载体)/`cgat_`(adapter gate receipt)/`clfr_`(legacy partition freeze receipt;窄审 r7 修复②)/`cear_`(resolution winner registry;窄审 r8 修复③)/`crca_`(resolution conflict audit;r10-b1 修③)——共 14 枚;仅标识/关联键,不构成 authority(§1.3 通则)。**[spec 期实现细节]**
- **最小索引集(窄审 r1 修复 D2-11;r2 修复④:删除引用不存在列的 transition 索引——`UNIQUE(installationId, cutoverEpoch, expectedTransitionRevision)` 已充当查询锚,无 `transitionRevision` 列)**:admin receipts(expiresAtBroker;revocation state)/obligations 部分索引 `WHERE resolvedByRef IS NULL`(未终局扫描)/abort attempts 部分唯一即扫描锚/closures 六列 UNIQUE 即查询锚/coverage evidence 见其表行/janitor 各按时钟源列。**[spec 期实现细节]**
- **migration activation fence(窄审 r1 修复 D2-11)**:cutover janitor 行与相关 GC 启用=本批表/列 `min_writer` floor 生效后(镜像 architecture §11 GC activation fence 先例);coverage 判定 join 不依赖任何待回填列(全新表,零 backfill)。
- **`effect_intents` B1 四列 migration(legacy-v7 分区制;空表=平凡快路;窄审 r6 修复①)**:①expand:四列以 **nullable** 落列(additive,无 contract);②既有行(四列 NULL 即 legacy-v7 行,天然可判定)处置:**不回填、不重算既有 `effectSemanticDigest`、不删除、不改写 identity**——terminal 行保留 tombstone;live 行由 operator 按 baseline §12.3 状态机既有出口收口(cancelled/superseded 等),仍 durable;**legacy replay 纪律(r6-b1 修:marker 先于 digest)**:legacy marker 检查(四列 NULL)**置于 digest equality 分派之前**——同 toolUseId 命中 legacy 行即**无条件 4063+审计**,legacy 行不进入 digest equality replay(『7 与 11 字段 digest 恒不等』不作判据,已删);③**contract 形态=legacy-v7 分区 CHECK+冻结成员集 contract(r6-b1 二选一之①支;全局 NOT NULL 终点撤回;r7-b1 修②:补 frozen membership carrier+无插入窗次序+DB enforcement)**:DB CHECK=四列**同空同非空**(全 NULL=legacy-v7 分区;全非空=v11 行;partial 恒拒);**冻结次序(无插入窗证明)**=(i) `min_writer` floor 推进至本批生效(baseline architecture §11 惯例:旧 binary 拒启——floor 生效后不存在可写本表的 v7 writer,而 v11 broker 创建事务恒写四列非空(应用层 required),故四列全 NULL 行集自 floor 生效时点起封闭、不再增长);(ii) floor 生效后 broker 单 writer **legacy partition 冻结事务**(§17.3 增行;一次性):`count(*)` 现存四列全 NULL 行+按 `effectIntentId` 字典序实算 `frozenMemberSetDigest`(域=§16.3 `AgentBridge/LegacyEffectIntentPartition/v1`)+写 `effect_intent_legacy_freeze_receipts` 行 `{installationId, frozenAtBroker, frozenRowCount, frozenMemberSetDigest, minWriterFloor}`(五列,§17.1;每安装恰一行 append-only)——**boundary 的 exact 定义(r8-b1 修⑥)**:linearization point=冻结事务的 **DB commit 本身**(receipt 行与 enforcement 同事务原子可见,先后以事务序论,无「快照-约束」间隙);`frozenAtBroker`=同事务审计时钟申报、**非序锚**,`minWriterFloor`=(i) 已生效 floor 值;(iii) **DB enforcement(与 (ii) 同事务安装)**:冻结 receipt 提交后 INSERT 四列全 NULL 恒拒(存量 legacy 行已在表内、INSERT 仅产新行,故拒一切新全 NULL 行即把全 NULL 分区封闭为冻结成员集,无需逐行成员比对;`frozenMemberSetDigest` 供审计复核=成员集可机械重算比对)+INSERT partial NULL 恒拒(既有 CHECK)+四列 UPDATE immutable(legacy 行恒 NULL、v11 行恒创建事务值,禁改写 identity);**不设「全表无 NULL 后升全局 NOT NULL」终点**(非空旧库该终点因 tombstone 恒 durable 而不可达);④**B1 arm 前置谓词=joined effect-risk set(r6-b1 修;r7-b1 修①:unknown 支改 unresolved-risk 判定,raw-state presence 不再单独恒挡)**:存在下列任一行即 **不 arm**——(a) 四列 NULL 且 EffectIntent ∈ live;(b) joined EffectPermit ∈ {unused, executing};(c) joined EffectAttempt ∈ {intent_committed, executing};(d) joined EffectAttempt.state=unknown **且 `effect_attempt_resolutions` registry 无该 attempt 的 winner 行**(winner=UNIQUE(effectAttemptRef) first-winner,`factRef`→`crcr_`(pre-arm/armed)或 armed `cacr_`;**pre-arm 语境不产生 `cacr_`(abort=armed-only,r8-b1 修①)**;raw unknown 恒不覆写——阻挡判定=registry 层,与 architecture §16.1(四)drain 四合取④/C2 屏障同口径;矛盾提交=different-result conflict 拒绝,原 winner 不动;**winner 行 tagged cross-row binding 完整性异常→guard fail-closed 不放行(r9-b1 修②)**)(状态枚举=baseline spec:2032/2059 亲核在场;settleEffectAttempt 可同事务写 attempt=unknown+intent=settled 的绕过面由 (c)(d) 继续封闭,spec:2095);**registry winner 在场的 unknown 不挡 arm,其消费分两支**:v11 行(四列非空)→arm 事务 adoption 子步骤(§17.3 arm 行)以 winner 的 `exactSourceTuple` 精确匹配绑入新建 obligation(写 registry 行 `obligationRef`+`obligation.resolvedByRef`,fact 行不动);legacy-v7 行(四列 NULL)→本谓词直接消费 winner,winner 行全体按 resolutionId 字典序冻结入本次 fence 行 `legacyResolutionRefs[]` 并纳入 C2 `drainClosureDigest` 输入(不为 legacy 伪造 tuple/source-event obligation);**另:legacy-v7 分区非空(存在四列全 NULL 行)而 `effect_intent_legacy_freeze_receipts` 无本安装行时恒不 arm**(contract 前置,见③);fail-closed 方向:不 arm 不放行,terminal intent 不单独豁免;⑤空表快路:floor 生效后冻结事务 `count(*)=0` → receipt 仍写(`frozenRowCount=0`、`frozenMemberSetDigest`=空成员集实算值)→四列直接 NOT NULL+required(空表无 legacy 分区,enforcement 等价全表非空),②④ 的 legacy 分量退化为空操作。**[spec 期实现细节;r6-b1 三项+r7-b1 修①②open_objections 逐项落位]**
...
| GV-8 | `AgentBridge/EffectSemantic/v1` | 11 字段 replay-identity 构造值(baseline 7 字段+B1 四元组;完整 JCS 字节以 `docs/gen_golden_vectors.py` stdout 为权威) | `sha256:2dd7d2c19c1bd5a95c7fbefb4e0ab636d36e130ba4c76b5b17131aa365fd4f09` |

- **脚本 stdout 完整锚**:`python3 gen_golden_vectors.py` 输出的 SHA-256 = `d6c2bb5684a3fc05904cd2d1894a17b6bcea1cc9ba1e83749ba349ecdf140456`(GV-4/7/8 的完整 JCS 输入字节以脚本输出为权威,表内长输入仅示意)。

## 修订记录

- **v0.12.20(2026-07-31;B1 旧链退场 cutover 冻结稿 APPLY 合入)**:对审弧 B1 面 9 题 CONVERGED+冻结稿 r1-r14(codex-r14-b1 ACCEPT)——新增 §17 全章(21 表/C1-C5 封闭转移/writer 事务与 janitor/控制面零投影/migration+H-6 legacy 分区);§5.2 T3/T4 cutover fence guard+T2 `cutover_blocked`+§5.6 janitor 三行;§2.5/§4.4 接线句;§6.1 指针+§6.3 十三行;§12.2 不承载句+§12.3 B1 四元组列+effectSemanticDigest 11 字段+§12.10 行(差分 H;GV-8 新增,generator stdout 锚更新 d80341…→d6c2bb56…);§14 P0A-2b(标题十二项/登记行/HookLoadProofEvidence/三判据);§16.2 4065-4070 reserved 注记+4071-4078 八码;§16.3 五 domain+嵌套形状封闭句+EffectSemantic 行更新。**SEC-7 四组 gate 保持 open**(architecture §16.1(九));fixtures 义务=B1 冻结稿差分 G 八组(SEC-7 交付期,未随本版落文)。
- **v0.12.19(2026-07-18 会话 12;DS-1/DS-2 用户定案录入+DS-3 暂缓;零 wire 语义变化)**:用户原文「DS-1 采默认定案,DS-2 采默认定案,DS-3 暂缓」——§15 表 DS-1/DS-2 行标 ✅已定案+DS-3 行标暂缓+新增裁决记录 bullet(含 reopen 口径);同步三锚:§2.1 DS-1 行(已定案=Windows 恒 UNSUPPORTED)/§6.1 rooms 表 DS-2 标注/§7.2 dataClassification 注释;标题+状态行 v0.12.18→v0.12.19。DS-1/DS-2 裁决值=定案前默认安全侧(实现依据从「定案前临时」转「已裁决正式」);DS-3 未定,定案前行为继续按默认安全侧;报文形状/状态机/注册表/error 表零改动;GV 锚不变(d80341b0…)。DS-3 关联条 §9.1-§9.3 全部不动(维持 PENDING/UNSUPPORTED)。经 Codex r99 差分复核(wire/锚/GV APPROVE;DS-3 时序扩写+§15 导语两处录入问题抓出修正)+r100 极窄复核。
