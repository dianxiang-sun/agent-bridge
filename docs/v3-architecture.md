...
> **状态**:DRAFT v0.11(2026-07-28)。**v0.10→v0.11:v3 修订提案稳定集落盘**(proposal=repo 外 `agentbridge_v3_apply_2026-07/proposal/v3-revision-proposal-2026-07.md` r12,SHA-256 `351e4667e3565ecc762774d5a242ca664b6f36b2fbe1e3b99164be8e5b1f70a9`;Codex r11 复审 **ACCEPT**,verdict 绑定 r11 SHA `f60666c0d25b435812dfc845bb53558635c18e0f91b063f1731057ed494797b1`;评审链=r1-r12,盲审 2 轮全量+窄差分循环)。修入:P0A-2 FAIL 回写(F6/DR-7 升格+§16 local 源升级交付节)、worker 工作区与凭据策略(§9.2)、P0A-1 降级决策卡与探测扩展(§15)、deny-only 期 PEP 裁决(§10.1:观察模式两支否决/放行白名单总谓词/请求侧自报位=coordinated breaking upgrade/四类 semantic-deny 投影/G-5 前置开放项 F4)、verdict predicate 发起面默认(§6.4)、挂起保留/通知钟(§6.7 新增)、多项目 interim 隔离与 local 源升级(§16)、re-pin 继承(DR-2)、retention 分层与 `evidence_retention_gc` 不变量(§11)、卸载与 purge(§11)、doctor 矩阵(§12)、授权面板 G-5 伴生+恒拒清单只读说明形态(§10.1)、预算放宽配置项(§6.6)、same-UID 文档化(§10.4)、hook 面积稳定性(§9.1)、skills 层归属(§0)、新 provider 注册表清单与 DR-9 候选(§18-Q8)。**四面降级为 v0.12 定向提案 B1-B4 并登记 §18-Q9..Q12**(B1=旧链退场运维、B2=caller 完成唤醒/通知管道(UX-1 open)、B3=worker attestation(SEC-1 open)、B4=仲裁 ArbitrationContext(FM-2 open));相应能力在 v0.11 **不交付**,流程=设计对审先行。不重开 v0.9/v0.10 锁定决策。
...
- **(v0.11/2.4)deny-only 期与用户本人 APPLY 工作流裁决**:
  ...
  - **(iii) 观察模式=条件候选(不入 v0.11)**:前置=§15-9 HostTurnBoundary `host_turn_id` 支 PASS;过门后可重开三值 origin 设计(`unknown` 恒 deny;observe 仅 `positively_attested_human` 且非 policy_control),届时须一并交付 wire 扩展与跨域 cutover 协议。旧审批链整链退场的运维协议=§18-Q9(B1,v0.11 不交付)。
...
- **protectedEffectWrite 放行腿(§10.1 两腿之腿 1)= 独立 composite gate G-5**(完整组件集以 §15 G-5 为单一 SSOT,不在此复述;一并 hard-disabled 构建,过 G-5 conformance 后**单开关原子启用**;与上述 adapter canary enable 解耦)。RootAdmission/TaskLineage/最低 structural budget(§6.6)随 Phase 1B Task Core 落地;API billing 仍在其 conformance 全过前 hard-disabled。
...
- Q9(v0.11 降级,B1):**旧审批链退场运维**——durable cutover 状态机、活跃 session 覆盖证明(hook attestation 载体缺位:SessionIdentityEvidence 只证 session handle,「已加载 hook digest」carrier 须新设计)、quiescence 范围(`not_protected` 无 EffectAttempt、`unknown` 长期对账)、与 G-5 的次序绑定。SEC-7 保持 open;设计笔记=proposal 2.4(iv) 快照。
