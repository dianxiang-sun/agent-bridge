...
>
> ⚠**CURRENT LIVE ENTRYPOINT = §T.7 + ledger 入口 v16**(2026-07-20 会话 17 收尾:**v6 watchdog 修复 SEALED v6 RESEAL**[card 39f172dc→0f2ecfd5(会话18 账实修)/watchdog 2085754e;探测 default-channel wire 双 blocker→用户选 B 路径[rebuild bundle+重启 daemon]→watchdog 修复经 7 轮 Codex R1-R7 达 READY]→**首件=用户 v6 reapproval**→B 剩余[rebuild bundle 上线多通道 v1+重启 daemon+真机验+探测];入口 v16 核验块实测 ALL_CHECKS_PASS;会话16 收尾历史详 §T.1-T.6;**批次 2 G6 修复 SEALED v5 RESEAL[card 314b7e47,四轮 Codex 审:交接盲审+r121+r122+r123]+✅用户 v5 reapproval+✅§T commit+✅执行日 setup 六门 PASS(G5/G6 真机验)+✅device-auth(订阅);⚠driver --arm 探测 BLOCKED on watchdog default-channel healthz/恢复线 wire 不兼容(第四执行日发现:legacy healthz 缺 channelId/controlPort→恢复谓词必败+watchdog:171 abg-restart @default 被拒;三基线本身可构造 default/4502/pid,watchdog 能 arm——非命令构造不出/非 driver P12 报错)→下会话核心=Codex 协作修 watchdog healthz/pre-arm/abg-restart @default+CWD 固化→v6 reseal+用户 reapproval→重跑探测**;系统=部署态[sudoers lease 3h 自过期,零探测/零计费])。下会话激活唯一入口是 **§T**(→ ledger `~/Desktop/agentbridge_v3_apply_2026-07/ledger.md` **激活入口 v16**+**追记7/8/9[执行日探测尝试+盲审修补+会话17 v6 修复全过程]**;v15 及更早 SUPERSEDED,含 §S);**§A–§S 状态与行动段均历史快照**,勿据以起步/勿据以重开已定决策。
> **恢复顺序(现行)**:直接读 §T(**§T.7 会话17 v6 reseal=最新 live**;§T.1-T.6 会话16 历史;**§K.4 授权口径与历代教训继续有效**)+ledger 追记7/8/9+**入口 v16(唯一激活入口)**。§I.3 证据台账、§I.6b 五仓 pinned SHA、§F/§G(除后段明示替换处)仍有效。
...
读 ledger「下会话激活(入口;**v15**)」段并原样实跑其 fail-closed 核验块(merge-base 锚 9133dc3;精确 pin card 314b7e47/setup/watchdog+两 manifest 逐行)。⚠会话 14 曾 fabricate 一次工具输出——**每个 SHA/状态必须真实命令观察 function_result**。**⚠post-execution 态(会话16 已跑 reapproval/§T commit/setup 六门 PASS/device-auth;探测 BLOCKED on watchdog wire)——下方 §T.2 DONE/§T.3 状态含执行前快照,以 §T.6 为准**。**首件=系统处置(视用户:teardown 撤 lease 或留 3h 自过期)→核心=Codex 协作修 watchdog default-channel wire(双 blocker,见 §T.6)+CWD 固化→v6 reseal+用户 v6 reapproval→重跑 setup→探测**。**勿把 §T.1 读回 reapproval/setup 首件——那些会话16已完成(§T.6)**。
...
- **⚠card:3 措辞已知例外(erratum;s16-handoff 盲审)**:card:3 状态行的 r122「A-D/F/H 生产路径部分 CONFIRMED」+r123「A-F CONFIRMED」措辞略夸大(r123 精确=测试网 A-D/F CONFIRMED+E card 行号 finding+总 verdict NOT-READY,E 修后达 READY)。**card 故意不 churn**——用户 v5 reapproval 的对象锚(reapproval_v5.md:10)锚定 card SHA 314b7e47,改 card 破该审批对象锚需重 reapproval;driver 机器门只核 matrix pin d6ecffd8(driver:1222-1229)不核 card 文件 SHA。**精确口径以本 §T+ledger 追记8 为准=r123 A-D/F CONFIRMED+E finding**。
...
### T.7 会话 17 更新（v6 watchdog 修复 SEALED v6 RESEAL;2026-07-20;**supersede §T.6 的"下会话=Codex 协作修 watchdog wire"——那已在会话17 完成**）

> 一句话:§T.6 的 watchdog default-channel wire 双 blocker 经会话17 修复——用户选 **B 路径**(rebuild bundle 让 /healthz 出 channelId/controlPort + 重启 daemon),watchdog 侧修复经 **7 轮 Codex 对抗(R1-R7)达 READY-FOR-RESEAL**,已 **SEALED v6 RESEAL**(card 314b7e47→39f172dc→0f2ecfd5[会话18 账实修 s17-handoff 5 findings] / watchdog 8bc32026→2085754e / 新增 unit_watchdog_v6.py 0f58d27f);**⚠待用户 v6 reapproval → B 剩余(rebuild bundle=上线多通道 v1 + 重启 daemon + 真机验 + 探测)**。全过程/SHA/待做详见 **ledger 追记9 + 入口 v16**(唯一激活入口,此处不复制防漂移)。

- **双 blocker(现盘 spot-check 属实)**:(a)运行 legacy default bundle /healthz(daemon.js:2322-2333)缺 channelId/controlPort→恢复谓词必败;(b)abg-restart `@default` reserved 拒。根因=运行 bundle(a56142b)落后 src 一个 commit bece57b 多通道 v1(src/daemon.ts:858-869 已含字段;`verify:plugin-sync` 实测失败证漂移)。
- **v6 watchdog 修复**(7 轮 Codex R1-R7,每轮 Claude spot-check+mutant):restore default 省@/`restore_exit` gate/recovery 严格 modern+type-strict/pre-arm 身份门/`${prc}`修/TMUX env-i 剥离/轮询+SIGKILL deadline+timed_out→124/两连绿/env-i 白名单固定 PATH(含 bun)+HOME/TMPDIR fail-closed/wire 绑定/curl 状态检查+--max-redirs/--noproxy。
- **SHA**:watchdog 2085754e / test 0f58d27f(64 用例)/ card 39f172dc→**0f2ecfd5**(SEALED v6;会话18 账实修 5 findings 后现值);driver/setup/billing/matrix/sidecar/p0a8 零改动,**frozen_manifest 非零改动**(watchdog SHA 更新 98cf6b02→a97fddaa)→matrix cardSha256 d6ecffd8 不变 driver 门 PASS。入口 v16 核验块**实测 ALL_CHECKS_PASS**。
- **待做**:①**首件=用户 v6 reapproval**(誊 reapproval_v6.md 锚会话18 账实修后 card SHA=0f2ecfd5d4eff2ef0201817c3b73e8b9df62f1b859d4aa1d9c85da97ba9a6d7a;审批只认用户本人原文,不代填)②B 剩余(需用户):rebuild bundle(=上线多通道 v1,走 agent-bridge repo PR push-gated,向后兼容 default)+重启 daemon(断 Codex TUI 可重连)+重冻 EXP_PID+setup 六门+pre-arm 真机验+探测。
- **系统态**:会话16末部署态(sudoers lease NOTAFTER=20260720T111309Z 已过期自失效+controller root:wheel+kit/mkt/baseline+abprobe 登录);会话17 未动实机(零探测/零计费)。
