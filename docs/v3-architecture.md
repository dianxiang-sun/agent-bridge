...
> **状态**:DRAFT v0.7(2026-07-15)。**六条载荷级决策 D-1..D-6 + MCP Tasks 处置全部定案**(D-4=用户终裁;余=定案轮 Claude+Codex 双路独立出案→4 轮红队盲审收敛,证据 docs/v3-survey-evidence.md)。**v0.6→v0.7**:对 committed 字节做全量多维终审(6 并行维度 agent + Codex 独立全文对抗),补修 v0.6 落稿遗漏——头号=D-1 push 原语/证据映射漏落(复发 v0.5 根因,已补 §6.2/§6.4)、effectiveVerificationPolicy 派生、protectedStateRevision 推进规则、activation ownership 转移、编号/引用/gate 一致性。§0-B 转决策记录,裁决正文落 §5/§6/§8/§9.2/§10/DR-2/DR-8。v0.5 说明保留如下。
...
CallerRequest(调用方填;request body 中出现任何 origin 类字段 = 直接拒绝):
  ...
  dataClassification, requestedVerificationPolicy(none|predicate|manual) + acceptanceSpec?,
...
EffectivePolicySnapshot(broker 判定后写入;调用方不能自称写操作可重试):
  effectiveApprovalPolicy, effectiveVerificationPolicy, dataDecision,
...
3. **push 证据→outcome 映射(D-1;§6.2 `reserve_push_dispatch` 配套)**:push 路径 executor 不调 wire `complete_task`,由持 `PushCompletionHandle` 的 broker 进程内 adapter 按已关联协议终态调同一 core handler 提议 outcome:匹配当前 `{executorGeneration, rpcRequestId}` 的合法 MCP result 且 `isError≠true` → 提议 `succeeded`;匹配请求的 `isError=true`/JSON-RPC error/确定 schema violation → 提议 `failed`;已确认 interrupt/cancel → broker 派生 `cancelled`(adapter 不得提交);EOF/崩溃/超时/错 generation/无法唯一关联 → 进 `reconciling`,不得猜;executor 文本自称"成功/已批准"仅是 payload,不参与裁决。adapter 只判协议调用是否明确正常返回,业务交付正确性仍归 verification;broker core 最终校验 fence/state 并按本节 receipt/tombstone 结单。
4. verification:仅 `outcome=succeeded && effectiveVerificationPolicy≠none` 进入 `pending`,谓词/人工通过 → `accepted`;其余 `not_applicable`。**`effectiveVerificationPolicy` 由 broker 从 `requestedVerificationPolicy`+安装 policy 派生(§6.2 EffectivePolicySnapshot),caller 请求值不能直接放宽**(防 caller 请求 `none` 绕过)。
...
- G-5(protectedEffectWrite gate,放行腿总闸):Approval Agent + EffectPermit + EffectMediator + 执行前 fence/deadline 赢家的联合 conformance;全过前受保护写放行腿 hard-disabled(单开关原子启用,§10.1/§16)。用例=两腿决策表逐支(G-4)+ fence 总出口五分类 + protectedStateRevision 推进→旧 permit stale + policy-control 恒拒。
...
- **protectedEffectWrite 放行腿(§10.1 两腿之腿 1)= 独立 composite gate G-5**(Approval Agent + EffectPermit + EffectMediator 一并 hard-disabled 构建,过 G-5 conformance 后**单开关原子启用**;与上述 adapter enable 解耦)。RootAdmission/TaskLineage/最低 structural budget(§6.6)随 Phase 1B Task Core 落地;API billing 仍在其 conformance 全过前 hard-disabled。
...
- 轮 11(committed 字节终审,2026-07-15):对已落盘 v0.6 做全量多维终审(6 并行维度 agent:落锚保真/交叉引用/跨节漂移/安全不变量对抗/冷读者完整性/术语一致 + Codex 独立全文对抗)。三路独立收敛抓出 D-1 push 机制落稿遗漏(复发 v0.5 根因);安全不变量七项经对抗判 SEALED。补修 P0×3(D-1 补 §6.2/§6.4、effectiveVerificationPolicy、protectedStateRevision 推进)+ 一批一致性项。v0.6→v0.7。
