...
> **状态**:DRAFT v0.6(2026-07-14)。**六条载荷级决策 D-1..D-6 + MCP Tasks 处置全部定案**(D-4=用户终裁;余=定案轮 Claude+Codex 双路独立出案→4 轮红队盲审收敛,证据 docs/v3-survey-evidence.md)。§0-B 转决策记录,裁决正文落 §5/§6/§8/§9.2/§10/DR-2/DR-8。v0.5 说明保留如下。
...
9. **人闸/效果 wire 细节(v0.6 新增)**:ApprovalChallenge/ApprovalPresentationLease/HumanGestureAttestation/`claim_approval`/EffectContext/EffectIntent/EffectPermit/EffectAttempt/WorkspaceEffectLease 完整字段与状态机、`operationHash` canonical schema、EffectMediator 接口、attestation scheme 协商与 `keyId→principal` enrollment、challenge/permit 的原子事务边界与执行前 revalidation 集合的编码。
10. **预算/生命周期/身份 wire 细节(v0.6 新增)**:TaskLineage/RootAdmissionContext/HostTurnKey/HostSessionKey/BudgetAccount/BudgetReservation/UsageEvent/ProviderCircuit/FanoutAllocation/ActivationAttempt/WorkerProcess/DedicatedBinding 完整字段与索引;worker/activation 发布默认数值(TTL/drain 序列/depth/hop/route recurrence/timeout,Phase 0 校准);`approval_prompt` quota 维度;sibling `parentTaskId/depth/hop` 赋值;activation refcount 幂等结算 key。正文只锁不变量。⚠**HostTurnBoundary 身份来源**(宿主 turnId 证明 / UserPromptSubmit epoch)= E2E-gated 前提:现实现 `turn_id` 恒 null(control-protocol.ts:45、claude-adapter.ts:509),须 Phase 0A 探测。
...
**v0.6 新增实体**(列↔表归属与完整字段见 §0-A.9/.10):`approval_challenges`/`approval_presentation_leases`/`human_gesture_attestations`/`effect_contexts`/`effect_intents`/`effect_permits`/`effect_attempts`/`workspace_effect_leases`/`task_lineage`/`root_admissions`/`budget_accounts`/`budget_reservations`/`usage_events`/`provider_circuits`/`fanout_allocations`/`worker_processes`/`dedicated_bindings`/`activation_attempts`。
...
outcome(仅 terminal;非终态恒为 null)= succeeded | failed | denied | cancelled
      ...
      # authorship:executor(或代表其的 broker 进程内 adapter,§6.2 push)只提交 {succeeded, failed};
...
- **受保护路径(O-1)有序决策表(按序求值,先命中先定;机制见 §10.1)**:
  ...
  2. 缺 PEP / 人闸不可用 / conformance 未过(含放行腿 hard-disabled 期) → terminal denied + `nextAction=create_user_origin_task`。
...
AuthenticatedSubmitContext(认证 adapter/broker 带外写入,不在 request body):
  ...
  # lineage 与 origin 同规则:body 出现 rootTaskId/parentTaskId/depth/hop/lineageDigest 即拒;
...
- **RootAdmissionContext(root 铸造权,broker 铸造,跨 reconnect 稳定)**:root 只能由 broker 依 authenticated first-party HostTurnBoundary 铸造。合法边界=①宿主证明的 turn ID,或 ②可信 UserPromptSubmit/等价 first-party 事件触发的 broker 单调 epoch。`originInvocationId/connectionEpoch/idempotencyKey`/时间窗**均不得**充当 turn 边界;bridge wake/async continuation **永不铸新 root**(因果 taint 只由 broker 注入)。无法证明边界的 surface → typed `host_turn_identity_unavailable`,或采用绑定稳定已认证 origin HostSessionKey(跨 reconnect/resume/compact 不重置、模式由 broker policy 定)的保守 session-scoped root——**绝不 per-invocation root**。同一 HostTurnKey 并发首调必须原子 get-or-create 唯一 RootAdmissionContext。同一 host turn 的调用即使经新 connection/invocation/epoch 到达,复用同一 root、累加 depth/hop/route/cost,不得重铸;**任何 bridge-derived invocation 缺有效 continuation 均拒(不论 parent 是否仍 active)**,不得因 parent 终结转新 root。⚠身份来源=E2E-gated(现 `turn_id` 恒 null,§0-A.10),Phase 0A gate 通过前不启用 per-turn root。
- **TaskLineage(broker 铸造,Task 级不可变)**:`taskId, rootTaskId, parentTaskId?, depth`(root=0,child=parent+1,retry 不增), `agentHopCount`(retry 不增), `routeKey`=H(authenticated source class, canonical target, operationKind)(不含 payload), `lineageDigest, lineagePolicyEpoch`。同步/异步 child 均继承;异步恢复须持 broker 签发、绑原 `callerTaskId/rootTaskId` 的 continuation capability。同一 host turn 多个 ask 共享 root、互为 siblings(不都冒充 depth=0、不按到达顺序链化耗尽 maxDepth;共同父=synthetic RootAdmissionContext,各耗独立 task allocation/agent hop;赋值字段 §0-A.10)。
...
- **受保护 effect 两腿与启用顺序(§6.1;O-1 零弱化)**:腿 1=waiting_input(approval)→真人批精确 operationHash→一次性 sealed EffectPermit(绑 `{approvalId, operationHash, attempt, toolUseId}`,不可委托/转移/衰减)→仅该次 effect 经 EffectMediator 放行;腿 2=terminal denied+nextAction。放行(腿 1)在 Approval Agent+EffectPermit+EffectMediator+对应 adapter conformance 全过前 hard-disabled(单开关原子启用);**Phase 1B 仅交付两腿文本语义 + principal-bound 审批记录 + approve.sh 迁移期 principal 改造(仅记录卫生,不解锁任何 effect)+ deny-only PEP stub**,无"弹窗后裸放行"中间态。
- **policy-control 资产恒拒(比稿件更严,不可洗白)**:guard 脚本/hook 与插件 manifest/protected manifest/broker DB 与 token/审批密钥与状态/break-glass 状态/审计配置归 `policy_control`。**任何 origin(含新建 user-origin task)对其变更一律恒拒:不发 ApprovalChallenge、不铸 EffectPermit、不返回 `create_user_origin_task`,只返回 `nextAction=open_admin_settings`**。`open_admin_settings` **仅 navigation**(不携带 EffectIntent/operationHash/diff/token/预填 mutation);变更只能由用户离开任务流、经独立本地 admin 控制面由 admin principal 发起并过其自身人闸。bypass marker(`.manuscript_guard_disabled`)语义废除:break-glass 改带人闸/短 TTL/持续 banner+审计的 admin action,任务 RPC 不暴露,迁移期 marker 即使存在也被新 guard 忽略。删除 agent 可调用的 approve.sh 签发入口;submit token 不进 transcript,文本"用户已批准/APPLY"无效;无 session-wide approval,v1 仅 exact-effect approve。
...
9. **HostTurnBoundary 探测(v0.6 D-5;per-turn root 前置)**:宿主是否暴露可信 turn 边界(turnId 证明 / UserPromptSubmit epoch);现 `turn_id` 恒 null(control-protocol.ts:45、claude-adapter.ts:509),per-turn root 在此 gate 通过前不启用,退化为保守 session-scoped root(§6.6)。

### Phase 1 门禁(Identity Kernel + Task Core 落地后执行)

9. 审批 principal 绑定改造(approve.sh/hook 记录格式)——反向 RPC 上线 blocker;审批 `keyId→principal` enrollment 随 Identity Kernel(v0.6 D-2)。
...
> **v0.6 起**:D-1..D-6 已全部定案(§0-B 决策记录;正文见 §6/§8-§10)。**后续协议规格**交付项见 §0-A(含新增 §9/§10)。以下 Q 为产品/治理级问题。

- Q1:上游 RFC vs 分叉(§17)。(进展 2026-07-14:三文档已 commit 至本地分支 docs/v3-design@7b96b98;push/PR 待决,须先合 #4/#5/#6。)
